...
После старта API: экран **«Вход»** → регистрация или логин. Подтверждение приёма на **Таймере** отправляет запись в **`POST /v1/patients/me/intake-events`**; просмотр — **«История приёмов»** (иконка на таймере и таблетках, пункт меню в профиле), маршрут `/intake-history`.
